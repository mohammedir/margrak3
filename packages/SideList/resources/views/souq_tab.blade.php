<form action="{{url("/")}}/saveSouqTab" method="post">
    {{csrf_field()}}
            ...
            <div class="market-single-line row">
                <div class="manage-content-title row">
                    <input {{-- {{$p->b_is_tag == 1 ? "checked" : ""}} --}}type="checkbox"
                           id="select_all_souq_{{$p->pk_i_id}}"
                           name="cat_souq_{{$p->pk_i_id}}"
                        ...
                        <div class="manage-types">
                            ...
                            <div class="types-show">
                                @foreach($c1->getChilds as $c2)
                                    <input {{$c2->b_show_in_souq_menu == 1 ? "checked" : ""}} type="checkbox"
                                           name="cat_souq_{{$c2->pk_i_id}}"
                                           class="my_e_check__souq{{$p->pk_i_id}}"
                                           value="Agree">
                                    <a>
                                                        <span>
                                                            @if($c2->s_pic != "")
...
<script>
    $(function () {
                @foreach($primCategory as $p)
        var select_all_{{$p->pk_i_id}} = document.getElementById("select_all_souq_{{$p->pk_i_id}}");
        var checkboxes_{{$p->pk_i_id}} = document.getElementsByClassName("my_e_check__souq{{$p->pk_i_id}}");
